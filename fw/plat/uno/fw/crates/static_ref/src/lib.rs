...
//!
//! Vendored from [tock](https://github.com/tock/tock) kernel utilities.
...
// SAFETY: `StaticRef` points to a fixed address that is valid for the
// entire program lifetime, but that alone does not make cross-thread use
// safe. Many targets use `StaticRef` for MMIO or other memory with access
// ordering requirements, aliasing constraints, or single-threaded firmware
// assumptions.
// `StaticRef` therefore does not itself promise `Send` or `Sync`;
// concrete wrapper types must opt in to any cross-thread guarantees
// individually, with justification appropriate to the underlying memory.
