...
impl<'a> CoseSign1Object<'a> {
    ...
    pub fn encode(&self, out: &mut [u8]) -> Result<usize, AttestationError> {
    ...
    pub fn decode(data: &'a [u8]) -> Result<Self, AttestationError> {
        // Check it begins with COSE_Sign1 tag.
        if data.len() < COSE_SIGN1_TAG_SIZE || data[0] != COSE_SIGN1_TAG {
            return Err(AttestationError::CborDecodeError);
        }

        Ok(minicbor::decode(&data[COSE_SIGN1_TAG_SIZE..])?)
    }
}
...
pub fn encode_sig_struct(
    ...
    encoder.bytes(body_protected)?.bytes(&[])?.bytes(payload)?;
...
pub fn encode_rsa_public(n: &[u8], e: &[u8], out: &mut [u8]) -> Result<usize, AttestationError> {
    let out_len = out.len();
