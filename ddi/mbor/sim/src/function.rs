...
fn encode_masked_key(
    ...
    let mut encoder = azihsm_ddi_mbor_codec::MborEncoder::new(&mut encoded_metadata, false);
    metadata
...
impl FunctionStateInner {
    ...
    fn mask_vault_entry(
        ...
        let metadata = DdiMaskedKeyMetadata {
            svn: Some(1),
            key_type: azihsm_ddi_mbor_types::DdiKeyType::try_from(entry.kind()).map_err(|_| ERR)?,
