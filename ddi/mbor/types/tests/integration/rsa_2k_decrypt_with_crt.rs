    ...
    ddi_dev_test(
        ...
        |dev, _ddi, _path, session_id| {
            let (_key_id_rsa2k_pub, key_id_rsa2k_priv_crt, _) =
                store_rsa_keys_crt(dev, session_id, DdiKeyUsage::EncryptDecrypt, 2, None);
            ...
            let resp = helper_rsa_mod_exp(
                ...
                MborByteArray::from_slice(&[0x1; 256]).expect("failed to create byte array"),
...
#[test]
fn test_rsa_2k_decrypt_message_data_below_lower_limit() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            ...
            if device_kind != DdiDeviceKind::Physical {
                tracing::info!(
                    "Skipped test_rsa_2k_decrypt_message_data_below_lower_limit for virtual device"
                );
                return;
            }
...
#[test]
fn test_rsa_2k_decrypt_message_data_above_upper_limit() {
    ddi_dev_test(
        common_setup,
        common_cleanup,
        |dev, _ddi, _path, session_id| {
            ...
            if device_kind != DdiDeviceKind::Physical {
                tracing::debug!(
                    "Skipped test_rsa_2k_decrypt_message_data_above_upper_limit for virtual device"
                );
                return;
            }
