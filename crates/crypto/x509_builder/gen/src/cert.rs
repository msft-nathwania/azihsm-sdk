...
//!
//! Creates valid X.509v3 certificates (Root CA, Intermediate CA, Leaf) with
...
/// Build an Intermediate CA certificate template.
///
/// A temporary root cert is created internally to act as the issuer. The
/// root's subject DN uses the issuer needle patterns so they appear in
/// the intermediate cert's issuer DN.
///
/// # Returns
/// A [`CertTemplateResult`] with 11 variable fields: PUBLIC_KEY, SERIAL_NUMBER,
/// NOT_BEFORE, NOT_AFTER, ISSUER_CN, SUBJECT_CN, ISSUER_SN, SUBJECT_SN,
/// SUBJECT_KEY_ID, AUTHORITY_KEY_ID, PATH_LEN.
pub fn build_intermediate_cert() -> CertTemplateResult {
    // Build a root CA to sign the intermediate with.
    // Root's subject CN/SN must use the issuer needles so they appear as
    // the intermediate's issuer DN.
    let (root_key, root_pubkey_bytes) = generate_p384_keypair();
    let root_subject = build_name(&issuer_cn_needle(), &issuer_sn_needle());
    ...
    let subject_cn = subject_cn_needle();
    let subject_sn = subject_sn_needle();
    let inter_subject = build_name(&subject_cn, &subject_sn);

    let mut builder = X509Builder::new().expect("X509Builder");
    builder.set_version(2).expect("ver");
    set_serial(&mut builder, &serial);
    builder.set_subject_name(&inter_subject).expect("subj");
    builder
    ...
    builder.set_pubkey(&inter_key).expect("pubkey");

    // CA:TRUE with pathlen=5 as needle
    add_ca_extensions(&mut builder, Some(&root_cert), Some(5));
    add_aki(&mut builder, &root_cert);
    ...
    let ski_value = compute_ski(&inter_pubkey_bytes);
    let aki_value = compute_ski(&root_pubkey_bytes);

    // PATH_LEN: search for INTEGER encoding of 5 (02 01 05)
    let pathlen_needle = [0x02, 0x01, 0x05];
    let pl_offset = tbs::find_needle(&tbs_bytes, &pathlen_needle, "PATH_LEN_TLV");

    let fields = vec![
        FieldOffset {
            name: "PUBLIC_KEY",
            offset: tbs::find_needle(&tbs_bytes, &inter_pubkey_bytes, "PUBLIC_KEY"),
            len: P384_PUBKEY_LEN,
        },
        FieldOffset {
            name: "SERIAL_NUMBER",
        ...
        // Issuer CN/SN are unique (different needles from subject)
        FieldOffset {
            name: "ISSUER_CN",
        ...
        FieldOffset {
            name: "PATH_LEN",
            offset: pl_offset + 2, // skip tag+length, just the value byte
            len: 1,
        },
    ];
