...
/// NIST P-256 field prime `p` (big-endian).
const P256_PRIME: [u8; 32] = [
    0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
];

/// NIST P-384 field prime `p` (big-endian).
...
/// NIST P-521 field prime `p` (big-endian, 66 bytes): `2^521 - 1`.
...
impl EccCurve {
    ...
    /// Returns the big-endian byte representation of the field prime `p`.
    pub(crate) fn prime(&self) -> &'static [u8] {
        match self {
            EccCurve::P256 => &P256_PRIME,
            EccCurve::P384 => &P384_PRIME,
            EccCurve::P521 => &P521_PRIME,
        }
    }

    /// Returns `true` iff `coord` is a valid affine field element for this
    /// curve, i.e. `1 <= coord <= p - 1`, where `p` is the field prime.
    ///
    /// `coord` must be big-endian and exactly [`point_size`](Self::point_size)
    /// bytes long; any other length (or a value of `0` or `>= p`) returns
    /// `false`. This is the coordinate range check required before ECDH point
    /// validation (FIPS 186-5 / SP 800-56A public-key validation).
    pub fn coordinate_in_range(&self, coord: &[u8]) -> bool {
        if coord.len() != self.point_size() {
            return false;
        }
        // coord == 0 is rejected.
        ...
        // coord >= p is rejected (big-endian lexicographic compare, equal lengths).
        coord < self.prime()
    }
