...
use crate::decode::RequestView;
use crate::decode::ResponseView;
use crate::toc::*;
...
impl fmt::Display for RequestView<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for i in 0..self.toc_count() {
            write!(f, "  TOC[{}]: ", i)?;
            format_toc_entry(f, self.toc_entry(i))?;
        }
...
impl fmt::Display for ResponseView<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let status_name = match self.status() {
            0x0000_0000 => "Success",
            0x0000_0001 => "Unsupported Version",
            0x0000_0002 => "Invalid Opcode",
            0x0000_0003 => "Malformed Request",
            0x0000_0004 => "Internal Error",
            0x0000_0005 => "Session Not Found",
            0x0000_0006 => "Key Not Found",
            0x0000_0007 => "Permission Denied",
            _ => "Unknown",
        };
...
fn format_toc_entry(f: &mut fmt::Formatter<'_>, entry: TocEntry<'_>) -> fmt::Result {
    match entry {
